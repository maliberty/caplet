...
double ddot_(const int* n,      // dimension
...
float sdot_(const int* n,     // dimension
...
// dy <- dx
void dcopy_(const int* n,      // i
            const double* dx,  // i[]
            const int* incx,   // i
            double* dy,        // o[]
            const int* incy    // i
);
void scopy_(const int* n,     // i
            const float* sx,  // i[]
            const int* incx,  // i
            float* sy,        // o[]
            const int* incy   // i
...
// dx <- da dx
void dscal_(const int* n,      // i
            const double* da,  // i
            double* dx,        // io[]
            const int* incx    // i
);
void sscal_(const int* n,     // i
            const float* sa,  // i
            float* sx,        // io[]
            const int* incx   // i
);
...
// y <- alpha A(') x + beta y
void sgemv_(const char* trans,   // i 'n' or 't'
            const int* m,        // i
            const int* n,        // i
            const float* alpha,  // i
            const float* A,      // i[]
...
void dgemv_(const char* trans,    // i 'n' or 't'
            const int* m,         // i
            const int* n,         // i
            const double* alpha,  // i
            const double* A,      // i[]
...
// C <- alpha A(') B(') + beta C
void dgemm_(const char* transA,   // i	'n' or 't' of A
...
void sgemm_(const char* transA,  // i	'n' or 't' of A
            const char* transB,  // i	'n' or 't' of B
            const int* m,        // i	C:mxn
            const int* n,        // i	C:mxn
            const int* k,        // i	A:mxk, B:kxn
...
void dsysv_(const char* uplo,  // i	'u' or 'l'
            const int* n,      // i	number of linear equations
            const int* nrhs,   // i	number of rhs columns
            ...
            const int* ldb,    // i[]
            double* work,      // t[]	work[0] shows the optimal size when info
                               // == 0 on exit
...
void ssysv_(const char* uplo,  // i	'u' or 'l'
            ...
            const int* ldb,    // i[]
            float* work,       // t[]	work[0] shows the optimal size when info
                               // == 0 on exit
            int* lwork,        // i	lwork>=n*NB, the optimal blocksize of dsytrf
            int* info          // o	= 0: successful exit
                               //		< 0: if INFO = -i, the i-th argument had
...
inline float sdot(const int& n,     // dimension
                  const float* dx,  // []vector x
                  const int& incx,  // index increment of each access of x
                  const float* dy,  // []vector y
                  const int& incy   // index increment of each access of y
