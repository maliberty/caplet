...
void PanelRenderer::computeGLDisplayBoundary()
{
  ...
  for (unsigned int i = 0; i < rectGLForDisplay.size(); ++i) {
    for (RectangleGLList::const_iterator eachRectIt
         = rectGLForDisplay[i].begin();
         eachRectIt != rectGLForDisplay[i].end();
         ++eachRectIt) {
...
void PanelRenderer::resizeGL(int width, int height)
{
  ...
  if (hwRatio > yr / xr) {
    glOrtho(
        -xr, +xr, -xr * hwRatio, +xr * hwRatio, (-xr * zDepth), (+xr * zDepth));
  } else {
    glOrtho(
        -yr / hwRatio, +yr / hwRatio, -yr, +yr, (-yr * zDepth), (+yr * zDepth));
  }
...
void PanelRenderer::paintGL()
{
  ...
  if (isLoaded) {
    for (unsigned int i = 0; i < rectGLForDisplay.size(); ++i) {
      glMaterialfv(
          GL_FRONT, GL_DIFFUSE, getColor(renderPropertyVec[i].colorIndex));

      glBegin(GL_QUADS);
      for (RectangleGLList::iterator rectIt = rectGLForDisplay[i].begin();
           rectIt != rectGLForDisplay[i].end();
           ++rectIt) {
        glNormal3f(rectIt->xn, rectIt->yn, rectIt->zn);

        if (rectIt->xn != 0) {
          if (rectIt->shapeType == RectangleGL::FLAT_TYPE
              && rectIt->shapeShift != 0 && rectIt->xn > 0) {
          ...
          } else if (rectIt->shapeType == RectangleGL::FLAT_TYPE
                     && rectIt->shapeShift != 0 && rectIt->xn < 0) {
            glVertex3f(rectIt->x1 - shiftLarge, rectIt->y1, rectIt->z1);
            glVertex3f(rectIt->x1 - shiftLarge, rectIt->y2, rectIt->z1);
            glVertex3f(rectIt->x1 - shiftLarge, rectIt->y2, rectIt->z2);
          ...
          //                    else if
          //                    (rectIt->shapeType==RectangleGL::ARCH_TYPE &&
          //                    rectIt->xn > 0){
          //                        glVertex3f( rectIt->x1+shiftSmall,
          //                        rectIt->y1, rectIt->z1 ); glVertex3f(
          //                        rectIt->x1+shiftSmall, rectIt->y2,
          //                        rectIt->z1 ); glVertex3f(
          //                        rectIt->x1+shiftSmall, rectIt->y2,
          //                        rectIt->z2 ); glVertex3f(
          //                        rectIt->x1+shiftSmall, rectIt->y1,
          //                        rectIt->z2 );
          //                    }
          //                    else if
          //                    (rectIt->shapeType==RectangleGL::ARCH_TYPE &&
          ...
          if (rectIt->shapeType == RectangleGL::FLAT_TYPE
              && rectIt->shapeShift != 0 && rectIt->yn > 0) {
          ...
          //                    (rectIt->shapeType==RectangleGL::ARCH_TYPE &&
          //                    rectIt->yn > 0){
          //                        glVertex3f( rectIt->x1,
          //                        rectIt->y1+shiftSmall, rectIt->z1 );
          //                        glVertex3f( rectIt->x2,
          //                        rectIt->y1+shiftSmall, rectIt->z1 );
          //                        glVertex3f( rectIt->x2,
          //                        rectIt->y1+shiftSmall, rectIt->z2 );
          //                        glVertex3f( rectIt->x1,
          //                        rectIt->y1+shiftSmall, rectIt->z2 );
          //                    }
          //                    else if
          ...
          if (rectIt->shapeType == RectangleGL::FLAT_TYPE
              && rectIt->shapeShift != 0 && rectIt->zn > 0) {
          ...
          //                    (rectIt->shapeType==RectangleGL::ARCH_TYPE &&
          //                    rectIt->zn > 0){
          //                        glVertex3f( rectIt->x1, rectIt->y1,
          //                        rectIt->z1+shiftSmall ); glVertex3f(
          //                        rectIt->x2, rectIt->y1,
          //                        rectIt->z1+shiftSmall ); glVertex3f(
          //                        rectIt->x2, rectIt->y2,
          //                        rectIt->z1+shiftSmall ); glVertex3f(
          //                        rectIt->x1, rectIt->y2,
          //                        rectIt->z1+shiftSmall );
          //                    }
          //                    else if
          //                    (rectIt->shapeType==RectangleGL::ARCH_TYPE &&
          //                    rectIt->zn < 0){
        ...
        for (RectangleGLList::iterator rectIt = rectGLForDisplay[i].begin();
          ...
          if (rectIt->xn != 0) {
            if (rectIt->shapeType == RectangleGL::FLAT_TYPE
              ...
              glVertex3f(rectIt->x1 - shiftLarge, rectIt->y2, rectIt->z2);
              glVertex3f(rectIt->x1 - shiftLarge, rectIt->y1, rectIt->z2);
            } else if (rectIt->shapeType == RectangleGL::ARCH_TYPE
                       && rectIt->xn > 0) {
...
void PanelRenderer::conductorFPList2RectGLForDisplay()
{
  ...
  } else if (colorScheme == BYCONDUCTOR) {  //*group rects by conductor
...
void PanelRenderer::mousePressEvent(QMouseEvent* event)
{
  ...
  int lastRadius2 = static_cast<int>(std::sqrt(
      (lx - xCenter) * (lx - xCenter) + (ly - yCenter) * (ly - yCenter)));
