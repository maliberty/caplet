...
void MainWindow::displayPWCBasisFunction()
{
  ...
  const ConductorFPList& condList
      = geoLoader->getPWCBasisFunction(unit, suggestedPanelSize);
...
void MainWindow::displayInstantiableBasisFunction()
{
  ...
  const float mergeDist
      = ui->mergeDistLineEdit->text().toFloat() * mergeDistUnit;
  ...
  const ConductorFPList& condList = geoLoader->getInstantiableBasisFunction(
      unit, archLength, projDist, mergeDist);
  panelRenderer->loadGLRects(&condList);
}
...
void MainWindow::on_actionOpenFile_triggered()
{
  QString inputFileName = QFileDialog::getOpenFileName(
      ...
      tr("Open File"),
      QDir::currentPath(),
      tr("Caplet Geometry files [*.geo] (*.geo);;Fastcap files [*.qui] "
         "(*.qui);;All files (*.*)"));
  ...
  if (!fin.is_open()) {
    QMessageBox::critical(
        this, "File Not Found", inputFileName + " is not found.");
    fin.close();
    return;
  }
  fin.close();
...
void MainWindow::on_extractionButton_clicked()
{
  ...
  if (ui->solverFastcapRadio->isChecked()) {
    ...
    vector<float> errorVector
        = result.compareDiagonal(geoLoader->getReferenceResult());
    ...
    QString msg = QString("# (") + QDateTime().currentDateTime().toString()
                  + QString("): Done Cmat extraction using FASTCAP.");
    logTime(msg);
    ...
    QString msg = QString("# (") + QDateTime().currentDateTime().toString()
                  + QString("): Done Cmat extraction using CAPLET.");
    logTime(msg);
    log(pathFileBaseName);
    log(resultText);
    log(QString("Diagonal Error(%) = ") + errorVectorString);
    ...
    QString msg
        = QString("# (") + QDateTime().currentDateTime().toString()
          + QString(
              "): Done Cmat extraction using CAPLET with PWC basis functions ");
    logTime(msg);
    log(pathFileBaseName);
    log(resultText);
    log(QString("Diagonal Error(%) = ") + errorVectorString);
...
void MainWindow::on_computeReferenceButton_clicked()
{
  ...
  QString pathFileBaseName = canonicalPath + "/" + fileBaseName + "_"
                             + QString::number(pwcSize / unit);
  ExtractionInfo* thisResult
      = &(geoLoader->runFastcap(pathFileBaseName.toUtf8().data(), fastcapFlag));
  ...
  do {
    ...
    pwcSize /= alpha;
    geoLoader->getPWCBasisFunction(unit, pwcSize);
    QString pathFileBaseName
        = canonicalPath + "/" + fileBaseName + "_" + QString::number(pwcSize);
    thisResult = &(
        geoLoader->runFastcap(pathFileBaseName.toUtf8().data(), fastcapFlag));
...
void MainWindow::on_loadReferenceButton_clicked()
{
  ...
  if (count != geoLoader->getNumberOfConductor()) {
    QMessageBox::critical(
        this,
        "Reference Matrix Loading Failure",
...
{
  colorpalette
      = new ColorPalette(panelRenderer->color, panelRenderer->nColor, this);
