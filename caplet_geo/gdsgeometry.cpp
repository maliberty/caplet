...
//*
//*
Point::Point() : x(0), y(0), z(0), dir(FLAT), len(0)
{
}

Point::Point(int xx, int yy) : x(xx), y(yy), z(0), dir(FLAT), len(0)
{
}

Point::Point(int xx, int yy, int zz) : x(xx), y(yy), z(zz), dir(FLAT), len(0)
{
}
...
//*
//*
Polygon::Polygon(const PointList::allocator_type& allo) : PointList(allo)
{
}

Polygon::Polygon(Polygon::size_type n,
                 const Point& value,
...
Polygon::Polygon(Polygon::iterator first,
...
Polygon::Polygon(const Polygon& poly) : PointList(poly)
{
}
...
bool Polygon::isManhattan() const
{
  ...
  for (; eachVertexIt != this->end(); ++eachVertexIt, ++prevVertexIt) {
...
//**
//* Default constructor
Rectangle::Rectangle() : normal(FLAT), x1(0), x2(0), y1(0), y2(0), z1(0), z2(0)
{
}
...
{
  //* z-dir
  if ((this->normal == ZP || this->normal == ZM)
      && (rect.normal == ZP || rect.normal == ZM) && (this->z1 == rect.z1)
      && this->isOverlapping3d(rect) == true) {
    //* check: rect has no corner inside this
    if (rect.hasCornerInside(*this) == true) {
      cerr << "WARNING: Rectangle::operator- encounters corner inside cases "
           << endl;
    }
...
RectangleList::RectangleList(const allocator_type& allo) : list<Rectangle>(allo)
{
}

RectangleList::RectangleList(size_type n,
...
RectangleList::RectangleList(iterator first,
                             iterator last,
                             const allocator_type& allo)
...
RectangleList::RectangleList(const RectangleList& rectList) : list<Rectangle>()
{
...
void decomposeXdir(Rectangle& rectI,
                   Rectangle& rectJ,
                   RectangleList& decomposedRectJ);
void decomposeYdir(Rectangle& rectI,
                   Rectangle& rectJ,
                   RectangleList& decomposedRectJ);
void RectangleList::decompose()
{
  ...
  for (RectangleMap::iterator itemIit = rectMap.begin();
    ...
    for (RectangleList::iterator eachRectIt = decomposedRectJ.begin();
         eachRectIt != decomposedRectJ.end();
         ++eachRectIt) {
      rectMap.insert(
          pair<double, Rectangle>((*eachRectIt).area(), *eachRectIt));
    }
  }

  this->clear();
  for (RectangleMap::iterator itemIt = rectMap.begin(); itemIt != rectMap.end();
       ++itemIt) {
...
Conductor::Conductor() : nMetal(0), nVia(0), nLayer(0), layer(0)
{
}
...
Conductor::Conductor(int nMetal1, int nVia1)
    : nMetal(nMetal1),
      nVia(nVia1),
      nLayer(nMetal1 + nVia1),
      layer(nLayer, vector<RectangleList>(6))
{
}
...
{  ///
  ...
  for (int i = 0; i < nLayer; ++i) {
    for (int j = 0; j < nDir; ++j) {
      layer[i][j].insert(
          layer[i][j].end(), rhs.layer[i][j].begin(), rhs.layer[i][j].end());
    }
  }
  return *this;
}
...
bool Conductor::isContaining(const Rectangle rect, const int layerIndex)
{
  ...
  for (RectangleList::iterator eachRectIt = layer[layerIndex][TOP].begin();
       eachRectIt != layer[layerIndex][TOP].end();
       ++eachRectIt) {
...
//*   o. Modify via size if that exceeds the connected metal boundary
//(SHRINK_VIA)
//*      This is the only place that modifies rect.
Rectangle innerDecompose(RectangleList& rectList,
                         RectangleList::iterator& thisRectIt,
                         Rectangle viaRect);

void Conductor::generateVia(Rectangle rect,
                            const int viaIndex,
                            ...
                            const bool isDecomposed)
{
  ...
  //* generate top and bottom rectangle with decomposition
    ...
    for (int i = 0; i < numberOfCases; ++i) {
      ...
      for (RectangleList::iterator eachRectIt
           = layer[metalIndex][metalRectDir].begin();
           eachRectIt != end;) {
                         ...
                         Rectangle viaRect)
{
  ...
  if (thisRectIt->hasCornerInside(viaRect) == true) {
    cerr << "WARNING: generateVia needs to handle the corners inside via"
         << endl;
  }
  ...
  //* right rect
  RectangleList::iterator rightRectIt
      = rectList.insert(thisRectIt, *thisRectIt);
  rightRectIt->x1 = viaRect.x2;
  //* bottom rect
  RectangleList::iterator bottomRectIt
      = rectList.insert(thisRectIt, *thisRectIt);
...
bool Conductor::checkSelfOverlapping(const int* const* viaConnect)
{
  ...
  for (int totalIndex = nMetal; totalIndex < nLayer; ++totalIndex) {
    ...
    for (int dirIndex = 0; dirIndex < dirInterfaceArrayLength; ++dirIndex) {
      ...
      const int metalIndex
          = viaConnect[viaIndex][viaConnectIndexArray[dirIndex]];
      for (RectangleList::const_iterator eachViaRectIt
           = layer[totalIndex][dirVia].begin();
           eachViaRectIt != layer[totalIndex][dirVia].end();
           ++eachViaRectIt) {
...
bool RectangleGL::isOverlappingProjection(const RectangleGL& rect) const
{
  ...
  //* debug info
  cerr
      << "ERROR: impossible direction in RectangleGL::isOverlappingProjection()"
      << endl;
  cerr << "(" << xn << "," << yn << "," << zn << ")" << endl;
  return false;
}
...
{
  return (xn == rect.xn && yn == rect.yn && zn == rect.zn && x1 == rect.x1
          && x2 == rect.x2 && y1 == rect.y1 && y2 == rect.y2 && z1 == rect.z1
          && z2 == rect.z2);
}

bool RectangleGL::isCoincidental(const RectangleGL& support,
...
{
  if ((zn != 0 && zn == rect.zn && z1 == rect.z1 && x1 <= rect.x1
       && rect.x2 <= x2 && y1 <= rect.y1 && rect.y2 <= y2)
      || (yn != 0 && yn == rect.yn && y1 == rect.y1 && z1 <= rect.z1
          && rect.z2 <= z2 && x1 <= rect.x1 && rect.x2 <= x2)
      || (xn != 0 && xn == rect.xn && x1 == rect.x1 && y1 <= rect.y1
          && rect.y2 <= y2 && z1 <= rect.z1 && rect.z2 <= z2)) {
    return true;
  }
  return false;
}
...
RectangleGL RectangleGL::intersectArchOnFlat(const RectangleGL& flat) const
{
  ...
  if ((shapeDir == X_DECAY && shapeNormalDistance > 0
       && !(flat.x1 <= x1 && x1 < flat.x2))
      ||  //* decay in +x
      (shapeDir == X_DECAY && shapeNormalDistance < 0
       && !(flat.x1 < x2 && x2 <= flat.x2))
      ||  //* decay in -x
      (shapeDir == Y_DECAY && shapeNormalDistance > 0
       && !(flat.y1 <= y1 && y1 < flat.y2))
      ||  //* decay in +y
      (shapeDir == Y_DECAY && shapeNormalDistance < 0
       && !(flat.y1 < y2 && y2 <= flat.y2))
      ||  //* decay in -y
      (shapeDir == Z_DECAY && shapeNormalDistance > 0
       && !(flat.z1 <= z1 && z1 < flat.z2))
      ||  //* decay in +z
      (shapeDir == Z_DECAY && shapeNormalDistance < 0
       && !(flat.z1 < z2 && z2 <= flat.z2))) {  //* decay in -z
    result.xn = 0;
    result.yn = 0;
    result.zn = 0;
    return result;
...
void RectangleGL::printCapletLine(ostream& out) const
{
...
#endif
  out << setw(14) << setprecision(6) << this->x1 << setw(14) << setprecision(6)
      << this->x2 << setw(14) << setprecision(6) << this->y1 << setw(14)
      << setprecision(6) << this->y2 << setw(14) << setprecision(6) << this->z1
  ...
  out << this->shapeDir;
  out << setw(14) << setprecision(6) << this->shapeNormalDistance << setw(14)
      << setprecision(6) << this->shapeShift;
  out << endl;
}
...
ConductorFP::ConductorFP()
    : nMetal(0),
      nVia(0),
      nLayer(nMetal + nVia),
      layer(0, DirRectangleGLList(nDir, RectangleGLList()))
{
}
...
ConductorFP::ConductorFP(const int nMetal, const int nVia)
    : nMetal(nMetal),
      nVia(nVia),
      nLayer(nMetal + nVia),
      layer(nLayer, DirRectangleGLList(nDir, RectangleGLList()))
{
      ...
      layer(nLayer, DirRectangleGLList(nDir, RectangleGLList()))
{
  for (int layerIndex = 0; layerIndex < nLayer; ++layerIndex) {
    for (unsigned dirIndex = 0; dirIndex < nDir; ++dirIndex) {
      for (RectangleList::const_iterator eachRect
           = cond.layer[layerIndex][dirIndex].begin();
           eachRect != cond.layer[layerIndex][dirIndex].end();
           ++eachRect) {
...
ConductorFPList::ConductorFPList(size_type n,
...
ConductorFPList::ConductorFPList(iterator first,
                                 iterator last,
                                 const allocator_type& allo)
...
void ConductorFPList::constructFrom(const ConductorList& condList,
                                    const float unit)
{
  this->clear();
  for (ConductorList::const_iterator eachCond = condList.begin();
       eachCond != condList.end();
       ++eachCond) {
    this->push_back(ConductorFP(*eachCond, unit));
  }
}

ConductorFPList::ConductorFPList(const ConductorList& condList,
                                 const float unit)
    : list<ConductorFP>()
{
...
RectangleGLList::RectangleGLList(size_type n,
                                 const RectangleGL& value,
                                 const allocator_type& allo)
    : list<RectangleGL>(n, value, allo)
{
}

RectangleGLList::RectangleGLList(iterator first,
                                 iterator last,
                                 const allocator_type& allo)
...
void RectangleGLList::mergeProjection()
{
  ...
  for (iterator each = first; each != end(); ++each) {
    for (iterator after = first; after != end(); ++after) {
      ...
      if (each->xn * after->xn == 0 && each->yn * after->yn == 0
          && each->zn * after->zn == 0) {
        cerr << "ERROR: should be in the same direction (regardless of sign)"
             << endl;
        continue;
      }
  ...
  for (iterator each = first; each != end(); ++each) {
    ...
    for (iterator after = first; after != end(); ++after) {
      ...
      //* and projected from the same distance
        ...
        //* z-dir
        if (each->x1 == after->x1 && each->x2 == after->x2
            && abs(each->shapeNormalDistance - after->shapeNormalDistance)
                   < zero) {
          //* same xrange
          each->y1 = min(each->y1, after->y1);
          each->y2 = max(each->y2, after->y2);
          flagEraseAfter = true;
                   ...
                   && abs(each->shapeNormalDistance
                          - after->shapeNormalDistance)
                          < zero) {
          //* same yrange
          each->x1 = min(each->x1, after->x1);
          each->x2 = max(each->x2, after->x2);
        ...
        //* x-dir
        if (abs(each->shapeNormalDistance - after->shapeNormalDistance)
            < zero) {
          each->y1 = min(each->y1, after->y1);
          each->y2 = max(each->y2, after->y2);
          flagEraseAfter = true;
...
RectangleGLList::IteratorList
RectangleGLList::insertProjectedOverlappingRectangleGL(const RectangleGL& rect,
                                                       const float distance)
{
  RectangleGLList::IteratorList itList;

  RectangleGLList::iterator dummyEnd = this->insert(
      this->end(), RectangleGL());  // anchor for inserting projected rect
  for (RectangleGLList::iterator eachRectIt = this->begin();
       eachRectIt != dummyEnd && eachRectIt->shapeShift == 0;
       ++eachRectIt) {
    RectangleGL& eachRect = *eachRectIt;

    if (!(  //* x-dir normal
            (rect.xn > 0 && eachRect.xn < 0 && rect.x1 < eachRect.x1
             && eachRect.x1 < rect.x1 + distance)
            || (rect.xn < 0 && eachRect.xn > 0 && eachRect.x1 < rect.x1
                && rect.x1 < eachRect.x1 + distance)
            ||
            //* y-dir normal
            (rect.yn > 0 && eachRect.yn < 0 && rect.y1 < eachRect.y1
             && eachRect.y1 < rect.y1 + distance)
            || (rect.yn < 0 && eachRect.yn > 0 && eachRect.y1 < rect.y1
                && rect.y1 < eachRect.y1 + distance)
            ||
            //* z-dir normal
            (rect.zn > 0 && eachRect.zn < 0 && rect.z1 < eachRect.z1
             && eachRect.z1 < rect.z1 + distance)
            || (rect.zn < 0 && eachRect.zn > 0 && eachRect.z1 < rect.z1
                && rect.z1 < eachRect.z1 + distance))) {
      continue;
    }
...
void RectangleGLList::absorbCommonSupport(RectangleGLList::IteratorList& itList)
{
  ...
  for (IteratorList::iterator eachIt = itList.begin(); eachIt != --itList.end();
       ++eachIt) {
...
bool removeOneFarthestBadProjection(
    RectangleGLList& rectList,
    const RectangleGLList::iterator lastSupportIt,
    const float margin);
void RectangleGLList::removeBadProjection(float margin)
{
  //* Find the first projection
  iterator lastSupportIt = this->begin();
  for (; lastSupportIt != end() && lastSupportIt->shapeShift == 0;
       ++lastSupportIt) {
  }
    ...
    const float margin)
{
  ...
  for (RectangleGLList::iterator projectionIt = projectionList.begin();
       projectionIt != projectionList.end();
       ++projectionIt) {
    //* each copied projection
    for (RectangleGLList::iterator supportIt = rectList.begin();
         supportIt != firstProjectionIt;
         ++supportIt) {
      ...
      if (projectionIt->isCoincidental(*supportIt, margin)) {
        ...
        for (RectangleGLList::iterator compIt = firstProjectionIt;
             compIt != rectList.end();
             ++compIt) {
