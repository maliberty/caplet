...
{
 public:
  explicit GeometryNotManhattanError() : logic_error("") {}
...
{
 public:
  explicit ConductorLayerNotCompatibleError(const unsigned nMetal1,
                                            const unsigned nVia1,
                                            const unsigned nMetal2,
                                            const unsigned nVia2)
      : logic_error("")
  {
    ...
    m_what = "Conductor layer not compatible: " + m_layerInfo1
             + " != " + m_layerInfo2;
  }
...
class Point
{
 public:
  ...
  inline int vecLen2()
  {
    return (x != 0) ? int(std::abs(float(x))) : int(std::abs(float(y)));
  }  //* 2D Manhattan geometry only
};
...
{
 public:
  explicit RectangleList(const allocator_type& allo = allocator_type());
  explicit RectangleList(size_type n,
                         const Rectangle& value = Rectangle(),
                         const allocator_type& allo = allocator_type());
  RectangleList(iterator first,
                iterator last,
                const allocator_type& allo = allocator_type());
...
class RectangleGLList : public std::list<RectangleGL>
{
 public:
  ...
  explicit RectangleGLList(const allocator_type& allo = allocator_type());
  explicit RectangleGLList(size_type n,
                           const RectangleGL& value = RectangleGL(),
                           const allocator_type& allo = allocator_type());
  RectangleGLList(iterator first,
                  iterator last,
                  const allocator_type& allo = allocator_type());
  ...
  IteratorList insertProjectedOverlappingRectangleGL(const RectangleGL& rect,
...
class ConductorFPList : public std::list<ConductorFP>
{
 public:
  ...
  explicit ConductorFPList(size_type n,
